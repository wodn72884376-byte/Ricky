...
async function seedVariant() {
  const { rows } = await db.query<{ id: string }>(`
    ...
    p as (
      ...
      select b.id, '베타 LT 자켓', 'beta-lt-' || gen_random_uuid(), 'outerwear', 'CA', 'active' from b
      returning id
    )
  ...
  it('허용되지 않은 카테고리를 거부한다 — 내비게이션이 이 값으로 라우팅된다', async () => {
    await expect(
      db.query(`insert into products (brand_id, name, slug, category, status)
                select id, 'x', 'x-' || gen_random_uuid(), '아우터', 'active' from brands limit 1`),
  ...
  it('featured_rank는 양수여야 한다', async () => {
    await expect(
      db.query(`insert into products (brand_id, name, slug, category, featured_rank, status)
                select id, 'x', 'x-' || gen_random_uuid(), 'top', 0, 'active' from brands limit 1`),
    ).rejects.toThrow();
  });
});
